...
\section{My first latex document}
	\begin{enumerate}
		...
		\item Make a list of bullet points using the following code as a basis
		\begin{lstlisting}[gobble=12]
			\begin{enumerate}
				\item first item
			\end{enumerate}
		\end{lstlisting}
		\item Start a new section called `Science!'. Use the natbib package in combination with the physics.bib file to cite Einstein's mass-energy equivalence paper, which contains the famous equation $e=mc^2$. Use inline math mode to describe the equation. Refer back to the slides for guidance on which commands to use, and where to use them. Use the `agu' bibliography style.
		\item Use an equation environment to typeset the Schr\"{o}dinger equation: $$i\hbar \frac{\partial{}} {\partial{t}} \Psi(r,t) = \left[\frac{-\hbar^2}{2 \mu} \nabla ^2 + V(r,t)\right] \Psi(r,t)$$
