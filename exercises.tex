...
\section{Opening \TeX studio}
	\begin{enumerate}
		...
		\item You may need to configure the application on first run; accept the defaults. This uses the TeXLive \LaTeX \ distribution that has already been installed.
...
\section{Hello World!}
	\begin{enumerate}
		\item Create a new document. \texttt{File > New}.
...
\section{My first latex document}
	\begin{enumerate}
		...
		\item Open the documentation for the latex-document package: \\
...
\section{Download the University Thesis Document}
	...
	\begin{enumerate}
		...
		\item Use Windows explorer to navigate to the directory you just downloaded and extracted i.e. the directory which contains \texttt{thesis.tex}.
		
		Use the context menu (mouse right click) to open \textbf{Git Bash here}.
		
		You should see the prompt shows your directory in yellow:
		
		\begin{center}
			\includegraphics[width=\linewidth]{gitbash}
		\end{center}
		
		\item  Enter the following commands to configure Git, modified to use your name and email address:
		...
		\item Enter the following commands to create a new Git repository, save a snapshot of the files, and start tracking any changes you make to them:
...
\section{Customise the University Thesis Document}
	For the following exercises, recompile the \texttt{thesis.tex} document after you have completed each set of changes.
		\begin{enumerate}
			...
			\begin{lstlisting}[language=bash,showstringspaces=false,gobble=16]
			...
			\item In \textbf{Git Bash} enter the following command:
				\begin{lstlisting}[language=bash,showstringspaces=false,gobble=20]
